...
sbx_nestnet_b_rules "$SBX_B_PID" "127.0.0.2" "" "" || nest_fail "could not load the payload loopback rules."
sbx_nestnet_relays "" "" || nest_fail "could not start the host-port relays."
...
"@DNSMASQ@" \
    ...
    -d \
    --port=53 \
    --listen-address=127.0.0.2 --listen-address=10.200.0.1 \
    --bind-interfaces \
    --no-resolv \
    --no-hosts \
    --filter-AAAA \

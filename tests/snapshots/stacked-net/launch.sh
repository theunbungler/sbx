...
sbx_nestnet_b_rules "$SBX_B_PID" "127.0.0.2" "5433" "5353" || nest_fail "could not load the payload loopback rules."
sbx_nestnet_relays "5433" "5353" || nest_fail "could not start the host-port relays."
...
"@DNSMASQ@" \
    ...
    --no-hosts \
    --filter-AAAA \
    --server=/github.com/1.1.1.1 --nftset=/github.com/inet#sbx_filter#allowed4_1 --server=/google.com/1.1.1.1 --nftset=/google.com/inet#sbx_filter#allowed4_1 --server=/db.internal.example/9.9.9.9 --nftset=/db.internal.example/inet#sbx_filter#allowed4_1 --server=1.1.1.1 --nftset=//inet#sbx_filter#allowed4_1 \
